...
%% Identify closest point to inter-malleolus for force application

%Find the closest node on the tibia mesh to the IM landmark
imDist = distancePoints3d(V, landmarks.IM);
imPoint = V(find(imDist == min(imDist)),:);

%Find index in tibia surface that corresponds to points
logicAnkleNodes = ismember(V,imPoint,'rows');
logicAnkleFaces = all(logicAnkleNodes(Fb),2);
imPointID = find(V(:,1) == imPoint(:,1) & ...
    V(:,2) == imPoint(:,2) & V(:,3) == imPoint(:,3));

%Visualise to confirm
cFigure; hold on;
gpatch(Fb,V,'w','none',1);
plotV(imPoint,'r.','markerSize',15)
...
%File details
febioFebFileNamePart = 'tibiaModel_isoAnkleForce';
...
febio_spec.Mesh.NodeSet{1}.node.ATTR.id = imPointID(:);
...
%Ankle force point (anterior-posterior, medial-lateral directions)
...
febio_spec.MeshData.NodeData{1}.node.ATTR.lid = (1:1:numel(imPointID))';
